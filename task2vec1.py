...
class Task2Vec:
    ...
    def eval_classifier(self, lm_datasets: Dataset, batch_size = 128, classifier_opts: dict = None ):
        ...
        with torch.no_grad():
            for task_num in tqdm(range(num_tasks)):
                end_index = task_num * batch_size + batch_size
                if end_index > len(lm_datasets):
                    end_index = len(lm_datasets)
                task_dataset = lm_datasets.select(range(task_num * batch_size, end_index))

                device = next(self.model.parameters()).device
                inputs = {'input_ids': task_dataset['input_ids'].to(device),
                        'attention_mask': task_dataset['attention_mask'].to(device)}
                logits = self.model(**inputs, labels=inputs["input_ids"]).logits
                loss = self.loss_fn(logits, inputs["input_ids"], ignore_index=50256)
